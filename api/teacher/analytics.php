...
try {
    ...
    $stmt2 = $pdo->prepare("SELECT e.exam_id, e.exam_name, (SELECT COUNT(*) FROM exam_submissions WHERE exam_id=e.exam_id) AS submission_count, (SELECT AVG(CASE WHEN COALESCE(qtp.tp,0) > 0 THEN (es.score / qtp.tp) * 100 END) FROM exam_submissions es LEFT JOIN (SELECT exam_id, COALESCE(SUM(points),0) AS tp FROM questions GROUP BY exam_id) qtp ON qtp.exam_id=es.exam_id WHERE es.exam_id=e.exam_id) AS avg_score, (SELECT MAX(CASE WHEN COALESCE(qtp.tp,0) > 0 THEN (es.score / qtp.tp) * 100 END) FROM exam_submissions es LEFT JOIN (SELECT exam_id, COALESCE(SUM(points),0) AS tp FROM questions GROUP BY exam_id) qtp ON qtp.exam_id=es.exam_id WHERE es.exam_id=e.exam_id) AS highest, (SELECT MIN(CASE WHEN COALESCE(qtp.tp,0) > 0 THEN (es.score / qtp.tp) * 100 END) FROM exam_submissions es LEFT JOIN (SELECT exam_id, COALESCE(SUM(points),0) AS tp FROM questions GROUP BY exam_id) qtp ON qtp.exam_id=es.exam_id WHERE es.exam_id=e.exam_id) AS lowest FROM exams e WHERE e.class_id=?");
    ...
    foreach ($students as $student) {
        ...
        $stmt4 = $pdo->prepare("SELECT AVG(CASE WHEN COALESCE(qtp.tp,0) > 0 THEN (es.score / qtp.tp) * 100 END) AS avg FROM exam_submissions es LEFT JOIN (SELECT exam_id, COALESCE(SUM(points),0) AS tp FROM questions GROUP BY exam_id) qtp ON qtp.exam_id=es.exam_id WHERE es.user_id=? AND es.exam_id IN (SELECT exam_id FROM exams WHERE class_id=?)");
