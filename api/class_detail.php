...
// Get class info
$stmt = $pdo->prepare(
    'SELECT c.class_id, c.subject_code, c.subject_name, c.block, c.class_code, c.status,
...
addEffectiveArchiveFields($classInfo);

// Get exams for this class
$examStmt = $pdo->prepare(
    'SELECT e.exam_id, e.exam_name, e.duration_minutes, e.status, e.is_archived, e.archived_at,
            c.status AS class_status, c.is_archived AS class_is_archived,
            e.total_points, e.hold_scores, e.is_closed,
     ...
     WHERE e.class_id = :cid
      ORDER BY FIELD(CASE WHEN COALESCE(e.is_archived,0)=1 OR COALESCE(c.is_archived,0)=1
                          OR e.status=\'ARCHIVED\' OR c.status=\'ARCHIVED\' THEN \'ARCHIVED\' ELSE e.status END,
                    \'LIVE\', \'SCHEDULED\', \'DRAFT\', \'CLOSED\', \'ARCHIVED\'), e.exam_name ASC'
);
...
foreach ($exams as &$ex) {
    if ((int) ($ex['class_is_archived'] ?? 0) === 1 || strtoupper((string) ($ex['class_status'] ?? '')) === 'ARCHIVED') {
        $ex['is_archived'] = 1;
        $ex['status'] = 'ARCHIVED';
    } else {
        $ex['status'] = effectiveArchiveStatus($ex);
    }
    ...
    $scoresVisible = ((int) ($ex['is_closed'] ?? 0) === 1)
        ...
        || strtoupper((string) ($ex['status'] ?? '')) === 'ARCHIVED'
