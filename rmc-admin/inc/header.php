...
$user_thread = trim((string) ($admin['name'] ?? ''));
if ($user_thread === '') {
    $user_thread = trim(($admin['first_name'] ?? '') . ' ' . ($admin['last_name'] ?? ''));
}
if ($user_thread === '') {
    $user_thread = (string) ($admin['username'] ?? 'Admin');
}
$initials = strtoupper(substr(preg_replace('/[^A-Za-z]/', '', $user_thread), 0, 2) ?: 'AD');
...
$nav_items = [
    ...
    ['key' => 'assessments',  'href' => admin_url('assessments'), 'label' => 'Assessments',        'soon' => null, 'icon' => '<rect x="4" y="2.5" width="8" height="11" rx="1.4"/><path d="M6.5 5.8h3"/><path d="M6.5 8.3h3"/><path d="M6.5 10.8h1.8"/><path d="M9.3 14.2 10.4 15.5 13 12.6"/>'],
    ['key' => 'reports',      'href' => admin_url('reports'),     'label' => 'System-Wide Reports','soon' => null, 'icon' => '<path d="M4 13h8"/><path d="M5.4 7.4h1.6v5.6H5.4z"/><path d="M8.3 4.6h1.6v8.4H8.3z"/>'],
    ['key' => 'logs',         'href' => admin_url('logs'),        'label' => 'System Logs',        'soon' => null, 'icon' => '<circle cx="8" cy="8" r="5.6"/><path d="M8 5.2V8l2 1.6"/>'],
    ['key' => 'maintenance',  'href' => admin_url('maintenance'),'label' => 'System Maintenance','soon' => null, 'icon' => '<path d="M2.6 4.4h5.6"/><path d="M11.6 4.4h1.8"/><circle cx="9.8" cy="4.4" r="1.6"/><path d="M2.6 9.6h1.8"/><path d="M7.8 9.6h5.6"/><circle cx="6" cy="9.6" r="1.6"/><path d="M2.6 14.8h5.6"/><path d="M11.6 14.8h1.8"/><circle cx="9.8" cy="14.8" r="1.6"/>'],
];
...
<script>window.RMC_CSRF = <?php echo admin_json_for_script(admin_csrf_token()); ?>;</script>
...
<div class="app">
  <aside class="sidebar">
    ...
    <div class="sidebar-footer">
      ...
      <form class="logout-form" method="post" action="<?= e(admin_url('logout')) ?>">
        <input type="hidden" name="csrf_token" value="<?= e(admin_csrf_token()) ?>">
        <button class="logout-link" type="submit" title="Log out" aria-label="Log out">
