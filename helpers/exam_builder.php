...
/**
 * Canonicalize one builder row. Drafts may carry an explicit review flag, but
 * callers can still inspect `_validation_error` and decide whether to save it.
 */
function prepareBuilderQuestion(array $q, bool $publishing, int $index = 0): array {
    $type = normalizeQuestionType($q['question_type'] ?? '');
    if (!in_array($type, ['MC','TF','ID','ENUM'], true)) {
        throw new InvalidArgumentException('Choose a supported question type.');
    }
    $q['question_type'] = $type;
    $q['answer_matching'] = strtoupper((string) ($q['answer_matching'] ?? 'EXACT'));
    if (!in_array($q['answer_matching'], ['EXACT','IGNORE_CASE'], true)) {
        throw new InvalidArgumentException('Invalid capitalization setting.');
    }
    ...
    foreach ($allowed as $key) {
        if (array_key_exists($key, $rules)) $clean[$key] = (bool) $rules[$key];
    }
    ...
    $error = '';
    if (trim((string) ($q['question_text'] ?? '')) === '') $error = 'Question text is required.';
    elseif ((int) ($q['points'] ?? 1) < 1) $error = 'Points must be positive.';
    elseif (!empty($clean['review_required'])) $error = 'Confirm the imported question has been reviewed.';
    elseif ($type === 'MC' && (
        count($q['options']) < 2
        || in_array('', array_map('trim', $q['options']), true)
        || count(array_unique(array_map('trim', $q['options']))) !== count($q['options'])
        || !in_array(trim((string) ($q['correct_answer'] ?? '')), array_map('trim', $q['options']), true)
    )) $error = 'Fill unique choices and select a valid correct answer.';
    elseif ($type === 'TF' && !in_array($q['correct_answer'] ?? '', ['True', 'False'], true)) $error = 'Select True or False.';
    elseif ($type === 'ID' && trim((string) ($q['correct_answer'] ?? '')) === '') $error = 'Correct answer is required.';
    elseif ($type === 'ENUM' && (!$q['options'] || in_array('', array_map('trim', $q['options']), true))) $error = 'Fill every required enumeration answer.';

    if ($publishing && $error !== '') throw new InvalidArgumentException('Q' . ($index + 1) . ': ' . $error);
    if ($error !== '') $q['_validation_error'] = $error;
    return $q;
}
...
function prepareBuilderQuestions(array $questions, bool $publishing): array {
    ...
    foreach ($questions as $i => &$q) {
        ...
        $q = prepareBuilderQuestion($q, $publishing, (int) $i);
    }
    unset($q);
    return $questions;
}
